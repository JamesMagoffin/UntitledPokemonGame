...
class PokeBattle_Move_0A0 < PokeBattle_Move
    ...
    return true if attacker && (isConst?(thismove.id,PBMoves,:SURGINGSTRIKES) ||
                                isConst?(thismove.id,PBMoves,:WICKEDBLOW))
    return false
  end
end
...
class PokeBattle_Move_0C0 < PokeBattle_Move
  ...
  def pbNumHits(attacker)
    ...
    ret=3 if isConst?(@id,PBMoves,:WATERSHURIKEN) && 
             ...
             isConst?(attacker.species,PBSpecies,:GRENINJA) &&
             attacker.form==1
    ret=3 if isConst?(@id,PBMoves,:SURGINGSTRIKES)
    return ret
  end
end
