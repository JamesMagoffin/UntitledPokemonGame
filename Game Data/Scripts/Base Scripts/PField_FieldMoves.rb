...
def Kernel.pbSurf
  ...
  movefinder = Kernel.pbCheckMove(move)
  if !pbCheckHiddenMoveBadge(BADGEFORSURF,false) || (!$DEBUG && !movefinder) || !PBTerrain.isWater? #edithere
    return false
  end
...
HiddenMoveHandlers::ConfirmUseMove.add(:TELEPORT,proc{|move,pkmn|
   ...
   #return Kernel.pbConfirmMessage(_INTL("Want to return to the healing spot used last in {1}?",mapname)) # edit here
})
