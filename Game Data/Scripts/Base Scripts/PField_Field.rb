...
Events.onStepTakenFieldMovement+=proc {|sender,e|
  ...
  if $scene.is_a?(Scene_Map)
    ...
    elsif PBTerrain.isRedFlower?(pbGetTerrainTag(event,true))  # Won't show if under bridge
      $scene.spriteset.addUserAnimation(GRASS_ANIMATION_ID,event.x,event.y,true,1)
    elsif PBTerrain.isWhiteFlower?(pbGetTerrainTag(event,true))  # Won't show if under bridge
      $scene.spriteset.addUserAnimation(GRASS_ANIMATION_ID,event.x,event.y,true,1)
