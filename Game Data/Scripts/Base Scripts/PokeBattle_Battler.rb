...
################################################################################
  ...
  def pbSuccessCheck(thismove,user,target,turneffects,accuracy=true)
    ...
    if target.pbOwnSide.effects[PBEffects::CraftyShield] && thismove.pbIsStatus? &&
       thismove.function!=0xE5 && !unseenfist # Perish Song
    ...
    if target.effects[PBEffects::SpikyShield] && thismove.canProtectAgainst? &&
      ...
      if thismove.isContactMove? && !user.hasWorkingAbility(:LONGREACH) && 
         !user.hasWorkingItem(:PROTECTIVEPADS) && !user.fainted?
        @battle.scene.pbDamageAnimation(user,0)
    ...
    if target.effects[PBEffects::BanefulBunker] && thismove.canProtectAgainst? && 
      ...
      if thismove.isContactMove?(user) && !user.hasWorkingAbility(:LONGREACH) && 
         !user.hasWorkingItem(:PROTECTIVEPADS) && !user.fainted? && user.pbCanPoison?(nil,false)
