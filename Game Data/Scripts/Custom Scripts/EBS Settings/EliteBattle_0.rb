  ...
  def pbStartBattleCore(canlose)
    ...
    #========================
      if @party2.length==1
        ...
        $DiscordRPC.state = (_INTL("Level {1} {2}",wildpoke.level,wildpoke.name))
        $DiscordRPC.details = "Wild Pokémon Battle"
        $DiscordRPC.large_image = "wild"
        if wildpoke.isShiny?
          $DiscordRPC.state = (_INTL("Level {1} Shiny {2}",wildpoke.level,wildpoke.name))
          $DiscordRPC.small_image = "shiny"
        end
        if wildpoke.species == 151
          $DiscordRPC.details = "Mythical Encounter"
          $DiscordRPC.state = (_INTL("Level {1} {2}",wildpoke.level,wildpoke.name))
          $DiscordRPC.large_image = "mew"
        end
        $DiscordRPC.update
    ...
    #======================================
      ...
      @scene.pbStartBattle(self)
      @scene.sendingOut=true
      $DiscordRPC.state = (_INTL("{1}",@opponent.fullname))
      $DiscordRPC.details = "Trainer Battle"
      $DiscordRPC.large_image = "trainer"
      $DiscordRPC.update
      pbDisplayPaused(_INTL("{1}\r\nwould like to battle!",@opponent.fullname))
      @scene.smTrainerSequence.finish if @scene.smTrainerSequence
