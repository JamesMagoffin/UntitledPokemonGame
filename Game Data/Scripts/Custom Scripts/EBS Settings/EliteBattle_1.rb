...
class PokemonEvolutionScene
  ...
  def pbEvolution(cancancel=true)
    ...
    $DiscordRPC.state = (_INTL("{1} is evolving?",PBSpecies.getName(@pokemon.species)))
    $DiscordRPC.details = "Evolution"
    $DiscordRPC.update
  ...
  def createEvolved
    ...
    @sprites["msgwindow"].visible = true
    $DiscordRPC.state = (_INTL("{1} evolved into {2}",PBSpecies.getName(@pokemon.species),newspeciesname))
    $DiscordRPC.details = "Evolution"
    $DiscordRPC.update
  ...
  def self.included base
    base.class_eval do
      ...
      def pbStartScene(pokemon)
        ...
        @viewport.z = 99999
        @viewport.color = Color.new(0,0,0,0)
        $DiscordRPC.state = "Huh? Egg is hatching!"
        $DiscordRPC.details = "Hatching"
        $DiscordRPC.large_image = "egg"
        $DiscordRPC.update
        # initial fading transition
        16.times do
          @viewport.color.alpha += 16
      ...
      def pbMain
        ...
        $DiscordRPC.state = (_INTL("{1} Egg hatched!",@pokemon.name))
        $DiscordRPC.details = "Hatching"
        $DiscordRPC.large_image = "egg"
        $DiscordRPC.update
