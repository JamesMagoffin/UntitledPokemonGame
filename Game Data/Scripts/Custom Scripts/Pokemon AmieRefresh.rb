################################################################################
#                     Pokemon Amie/Refresh Version 1.0
#                     by Pizza Sun
#                     Adapted from rigbycwts's script
################################################################################
=begin
...
ItemHandlers::UseOnPokemon.add(:AWAKENING,proc{|item,pokemon,scene|
   if pokemon.hp<=0 || pokemon.status!=PBStatuses::SLEEP
     ...
     Kernel.pbMessage(_INTL("{1} woke up.",pokemon.name)) if scene==nil
     next true
   end
})
...
ItemHandlers::UseOnPokemon.add(:ANTIDOTE,proc{|item,pokemon,scene|
   if pokemon.hp<=0 || pokemon.status!=PBStatuses::POISON
     ...
     Kernel.pbMessage(_INTL("{1} was cured of its poisoning.",pokemon.name)) if scene==nil
     next true
   end
})
...
ItemHandlers::UseOnPokemon.add(:BURNHEAL,proc{|item,pokemon,scene|
   if pokemon.hp<=0 || pokemon.status!=PBStatuses::BURN
     ...
     Kernel.pbMessage(_INTL("{1}'s burn was healed.",pokemon.name)) if scene==nil
     next true
   end
})
...
ItemHandlers::UseOnPokemon.add(:PARLYZHEAL,proc{|item,pokemon,scene|
   if pokemon.hp<=0 || pokemon.status!=PBStatuses::PARALYSIS
     ...
     Kernel.pbMessage(_INTL("{1} was cured of paralysis.",pokemon.name)) if scene==nil
     next true
   end
})
...
ItemHandlers::UseOnPokemon.add(:ICEHEAL,proc{|item,pokemon,scene|
   if pokemon.hp<=0 || pokemon.status!=PBStatuses::FROZEN
     ...
     Kernel.pbMessage(_INTL("{1} was thawed out.",pokemon.name)) if scene==nil
     next true
   end
})
...
ItemHandlers::UseOnPokemon.add(:FULLHEAL,proc{|item,pokemon,scene|
   if pokemon.hp<=0 || pokemon.status==0
     ...
     Kernel.pbMessage(_INTL("{1} became healthy.",pokemon.name)) if scene==nil
     next true
   end
})
...
#-----------------------------------------------------------------------------
  ...
  def pbStartScene(pokemon, partyIndex)
    ...
    def main
      ...
      @sprites["pokemon"].x=DEFAULTSCREENWIDTH/2
      @sprites["pokemon"].y = DEFAULTSCREENHEIGHT/2+30
      @sprites["pokemon"].z = 3
      ...
      @sprites["switch"].x=DEFAULTSCREENWIDTH
      @sprites["switch"].y = DEFAULTSCREENHEIGHT
      ...
      @sprites["back"].y = DEFAULTSCREENHEIGHT
          ...
          if !$mouse.isStatic? #Is the mouse moving?
            ...
            if @counter>=50 #after counter reaches number, hearts appear
              ...
              for i in 0...4 #creates the hearts
                ...
                @sprites["#{i}"].z=4
                @sprites["#{i}"].x=DEFAULTSCREENWIDTH/2+rand(70)-40
                @sprites["#{i}"].y=DEFAULTSCREENHEIGHT/2-rand(40)+30
              end
              15.times do
    ...
    def pbBackdrop #gets background based off location
      ...
      # Choose backdrop
      backdrop="Field"
      forestMaps = [37, 40, 41, 44, 45, 46, 49, 47, 50]
      deepForestMaps = [59, 60, 62, 63, 61, 64]
      iceMountainMaps = [68, 67, 69, 70, 71, 72, 73]
      caveMaps = [54, 53, 56, 55, 57, 58]
      ...
      if environ==PBEnvironment::Cave
        ...
        backdrop="Mountain"
    elsif forestMaps.include?(mapID)
        backdrop="Forest"
      elsif mapID == 64
        backdrop="Graveyard"
      elsif deepForestMaps.include?(mapID)
        backdrop="DeepForest"
      elsif iceMountainMaps.include?(mapID)
        backdrop="IceMountain"
      else
    ...
    def update_command
      ...
      if (@sprites["textbox"]!=nil)
        if !@sprites["textbox"].disposed?
          if $mouse.click?(@sprites["textbox"])
            ...
            $DiscordRPC.state = (_INTL("Playing with {1}",@pokemon))
            $DiscordRPC.details = "Amie"
            $DiscordRPC.large_image = "wild"
            $DiscordRPC.update
      ...
      if @sprites["feedshow"]!=nil
        ...
        if ($mouse.isStatic?)&&((Time.now.to_f-@time1.to_f)>0.4)&&(!@sprites["feedshow"].disposed?)
          for x in 0...@foodnum
            if $mouse.leftPress?(@sprites["food#{x}"])
              if (@pokemon.getFullnessLevel==5)
                @full=true
                @time3=Time.now
                pbDrawOutlineText(@sprites["itemcount"].bitmap,DEFAULTSCREENWIDTH/2-64,@sprites["feed"].x+30,128,32,@pokemon.name+" is full!",Color.new(248,248,248),Color.new(30,30,30),0)
                break
              end
...
#Following Pokemon Support
class DependentEvents  
  alias amie_talk_to_pokemon talk_to_pokemon if (defined?(talk_to_pokemon))
  def talk_to_pokemon
...
if $game_switches[Toggle_Following_Switch] && !$Trainer.party[0].fainted? && !$PokemonGlobal.bicycle
...
NOTE:
Remove code below if you DON'T want this in you PokeGear as an option
...
class PokemonPokegearScreen
  alias amie_pbStartScreen pbStartScreen
  def pbStartScreen
    commands = []
    cmdMap     = -1
    cmdPhone   = -1
    ...
    commands[cmdPokeref = commands.length] = ["Pokemon Refresh",_INTL("Pokemon Refresh")] if $Trainer.party.length>0
