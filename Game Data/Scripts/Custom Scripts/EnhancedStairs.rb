...
if defined?(No_Shadow_If_Event_Name_Has) && !defined?(UPDATED_FOR_STAIR_SCRIPT)
  raise "Update your Overworld Shadows script!\n\nYou are using an outdated " + 
      "version that is incompatible with the Enhanced Stairs script!"
end
  ...
  def check_event_trigger_auto
    if $game_map && $game_map.events
      for event in $game_map.events.values
        ...
        if @real_x / Game_Map.realResX == event.x &&
           @real_y / Game_Map.realResY == event.y
...
class Game_Character
  ...
  def on_middle_of_stair?
    ...
    if @stair_start_x > @stair_end_x
      return @real_x < (@stair_start_x * Game_Map::TILEWIDTH - @stair_begin_offset) * Game_Map::XSUBPIXEL &&
          @real_x > (@stair_end_x * Game_Map::TILEWIDTH + @stair_begin_offset) * Game_Map::XSUBPIXEL
    else
      return @real_x > (@stair_start_x * Game_Map::TILEWIDTH + @stair_begin_offset) * Game_Map::XSUBPIXEL &&
          @real_x < (@stair_end_x * Game_Map::TILEWIDTH - @stair_begin_offset) * Game_Map::XSUBPIXEL      
    end
  ...
  def slope(x, y, ypos = 0, yheight = 1, begin_offset = 0)
    @stair_start_x = self.is_a?(Game_Player) ? @x : (@real_x / Game_Map.realResX).round
    @stair_start_y = self.is_a?(Game_Player) ? @y : (@real_y / Game_Map.realResY).round
  ...
  def moving?
    if self == $game_player && $DisableScrollCounter == 1
      ...
      @view_offset_y ||= 0
      self.center(
          (@real_x + @view_offset_x) / 4 / Game_Map::TILEWIDTH,
          (@real_y + @view_offset_y) / 4 / Game_Map::TILEHEIGHT
      )
    end
    return stair_moving?
  end
  ...
  def screen_y
    real_y = @real_y
    if on_stair?
      if @real_x / Game_Map::XSUBPIXEL.to_f <= @stair_start_x * Game_Map::TILEWIDTH &&
         @stair_end_x < @stair_start_x
        distance = (@stair_start_x - @stair_end_x) * Game_Map.realResX -
            2.0 * @stair_begin_offset * Game_Map::XSUBPIXEL
        rpos = @real_x - @stair_end_x * Game_Map.realResX - @stair_begin_offset * Game_Map::XSUBPIXEL
        fraction = 1 - rpos / distance.to_f
        if fraction >= 0 && fraction <= 1
          diff = fraction * (@stair_end_y - @stair_start_y) * Game_Map.realResY
          real_y += diff
        ...
        if fraction >= 1
          ...
          @y = endy
          @real_y = endy * Game_Map.realResY
          @view_offset_y = 0 if SMOOTH_SCROLLING && self.is_a?(Game_Player)
          clear_stair_data
          return stair_screen_y
        end
      elsif @real_x / Game_Map::XSUBPIXEL.to_f >= @stair_start_x * Game_Map::TILEWIDTH &&
          @stair_end_x > @stair_start_x
        distance = (@stair_end_x - @stair_start_x) * Game_Map.realResX -
            2.0 * @stair_begin_offset * Game_Map::XSUBPIXEL
        rpos = @stair_start_x * Game_Map.realResX - @real_x + @stair_begin_offset * Game_Map::XSUBPIXEL
        fraction = rpos / distance.to_f
        if fraction <= 0 && fraction >= -1
          diff = fraction * (@stair_start_y - @stair_end_y) * Game_Map.realResY
          real_y += diff
          if self.is_a?(Game_Player)
            if SMOOTH_SCROLLING
              @view_offset_y += diff - (@stair_last_increment || 0)
